...
export class CloudModel extends Model <CloudModel> {
    ...
    async init () {
        ...
        await this.get_namespaces()
        await this.get_storageclasses()
    ...
    async get_cluster_config (cluster: Cluster): Promise<ClusterConfig> {
        const config = await request_json(`/v1/dolphindbs/${cluster.namespace}/${cluster.name}/configs`)
        return config
    }

    async update_cluster_config (cluster: Cluster, newconfig: ClusterConfig) {
        const response = await request_json(`/v1/dolphindbs/${cluster.namespace}/${cluster.name}/configs`, {
            method: 'put',
            body: newconfig,
        })
        return response
