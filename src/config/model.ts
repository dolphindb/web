...
import { DdbInt, DdbString, type DdbCallOptions } from 'dolphindb/browser.js'
...
class ConfigModel extends Model<ConfigModel> {
    ...
    async add_agent_to_controller (host: string, port: number, alias: string) {
        await this.invoke('addAgentToController', [new DdbString(host), new DdbInt(port), new DdbString(alias)])
    }
