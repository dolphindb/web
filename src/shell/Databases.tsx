...
function SetTableComment () {
    ...
    return <Modal 
        className='db-modal' 
        open={set_table_comment_modal_visible} 
        onCancel={() => { shell.set({ set_table_comment_modal_visible: false }) }} 
        title={t('设置注释')}
    >
        <Form
            labelWrap
...
export class PartitionFile implements DataNode {
    ...
    async inspect () {
        ...
        let obj = await model.ddb.call<DdbTableObj>(
            ...
            this.site_node !== model.node_alias ? { node: this.site_node } : { }
        )
