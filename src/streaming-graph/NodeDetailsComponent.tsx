import { Tabs, Descriptions, Table, Typography, Empty, Card } from 'antd';
...
export function NodeDetailsComponent({ selectedNode, id }: NodeDetailsComponentProps) {
  ...
  const renderMetrics = () => {
    ...
    if (error) return <Text type='danger'>Failed to load metrics data: {error.message}</Text>;
    
    if (!data || data.length === 0) return <Empty description='No metrics data available' />;
