...
export function SelectSider () {
    ...
    return <>
        <div className={`dashboard-select-sider ${folding ? 'dashboard-select-sider-folding' : ''}`}>
            <div className='dashboard-graph-items'>
                {Object.entries(GraphTypeName).map((graph, i) => {
                    return <div key={graph[0]} className='dashboard-graph-item grid-stack-item' data-type={graph[0]}><RiseOutlined /> {folding ? '' : graph[1]}</div>
                })}
            </div>
