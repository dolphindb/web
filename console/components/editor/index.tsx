...
import { Editor as MonacoEditor, loader, EditorProps } from '@monaco-editor/react'
...
interface Props extends Omit<EditorProps, 'defaultLanguage' | 'language'> {}
...
export function Editor (props: Props) {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (languages.getLanguages().find(l => l.id === 'dolphindb')) {
                set_inited(true)
                return
            }
            ...
            if (!wasm_loaded) {
                ...
                await loadWASM(
                    await fetch('./vendors/vscode-oniguruma/release/onig.wasm')
                )
            }
            ...
            languages.registerSignatureHelpProvider('dolphindb', {
                ...
                provideSignatureHelp (doc, pos, canceller, ctx) {
                    ...
                    const { func_name, param_search_pos } = find_func_start(
                        doc,
                        pos
                    )
                    ...
                    const index = find_active_param_index(
                        doc,
                        pos,
                        param_search_pos
                    )
                    ...
                    const signature_and_params =
                        get_signature_and_params(func_name)
        ...
        <div className='monaco-container'>
            <MonacoEditor
                ...
                options={{
                    ...
                    fontFamily:
                        "MyFont, Menlo, 'Ubuntu Mono', Consolas, 'Dejavu Sans Mono', 'Noto Sans Mono', PingFangSC, 'Microsoft YaHei', monospace",
                    fontSize: 16,
                    insertSpaces: true,
                    codeLensFontFamily:
                        "MyFont, Menlo, 'Ubuntu Mono', Consolas, 'Dejavu Sans Mono', 'Noto Sans Mono', PingFangSC, 'Microsoft YaHei', monospace",
...
class TokensProviderCache {
    ...
    async createEncodedTokensProvider (
    ...
    ): Promise<monacoapi.languages.EncodedTokensProvider> {
        ...
        return {
            ...
            tokenizeEncoded (
                line: string,
                state: monacoapi.languages.IState
            ): monacoapi.languages.IEncodedLineTokens {
...
function create_style_element_for_colors_css (): HTMLStyleElement {
    ...
    if (monacoColors)
        monacoColors.parentElement?.insertBefore(
            style,
            monacoColors.nextSibling
        )
    else {
...
function get_signature_and_params (func_name: string): {
    ...
    const para = docs[func_name]?.children.filter(
        para => para.type === 'grammer'
    )[0]
    if (!para) 
        return null

    // 找出语法内容块的第一个非空行
    const funcLine = para.children[0].value
